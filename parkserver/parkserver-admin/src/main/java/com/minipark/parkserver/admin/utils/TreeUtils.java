...
import com.minipark.parkserver.admin.utils.exception.TreeUtilsException;
...
public class TreeUtils
{
    ...
    {
        // 处理异常情况
        if (StringUtils.isBlank(options.getId()) || StringUtils.isBlank(options.getText()) || StringUtils.isBlank(options.getNodeLevel())
                || StringUtils.isBlank(options.getParentId()) || StringUtils.isBlank(options.getHasChild()))
        {
            throw new TreeUtilsException("请确认map 中有 id,text,hasChild,nodeLevel,parentId 参数");
        }
        ...
        {
            String rsStr = "{\"all\" : {\"name\": \"全部\", \"type\": \"folder\",\"additionalParameters\":{\"children\":" + treeJson.toString() + "}}}";
            return rsStr;
        }
    }
    ...
    {
        // 处理异常情况
        if (StringUtils.isBlank((String) map.get("id")) || StringUtils.isBlank((String) map.get("name"))
                || StringUtils.isBlank((String) map.get("superId")) || StringUtils.isBlank((String) map.get("type")))
        {
            throw new TreeUtilsException("请确�?map 中有 id,name,superId,type 参数");
        }
    ...
    {
        /*
         * 1、将�?��有子节点的分别放入各级别的MAP中，其实�?��级节点放在List�?2、将子节点放入父节点中�?
         */
    ...
    public static List getListForJqgridTree(TreeOptions options) throws TreeUtilsException
    {
        if (StringUtils.isBlank(options.getId()) || StringUtils.isBlank(options.getText()) || StringUtils.isBlank(options.getNodeLevel())
                || StringUtils.isBlank(options.getParentId()) || StringUtils.isBlank(options.getHasChild()))
        {
            throw new TreeUtilsException("请确�?map 中有 id,text,hasChild,nodeLevel,parentId 参数");
        }
    ...
    /**
     * @author: ZHANGJH
     * @date: 2014-9-9 下午4:23:28
     * @Description: 整理出jqgrid �?��的结�?
     * @param rsList
     * @param parentId
     * @param childMap
     * @return
    ...
    public static List getOrgListForJsTree(TreeOptions options, String orgRootCode) throws TreeUtilsException
    {
        // 处理异常情况
        if (StringUtils.isBlank(options.getId()) || StringUtils.isBlank(options.getText()) || StringUtils.isBlank(options.getNodeLevel())
                || StringUtils.isBlank(options.getParentId()) || StringUtils.isBlank(options.getHasChild()))
        ...
        for (Object obj : options.getDataList())
        {
            ...
            String parentId = getValue(obj, options.getParentId());
            if (StringUtils.isBlank(parentId) || orgRootCode.equals(nodeMap.get("id")))
            {
                nodeMap.put("parent", "");
            }
            else
